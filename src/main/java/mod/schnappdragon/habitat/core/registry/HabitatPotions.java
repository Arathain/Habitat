...
public class HabitatPotions {
    public static final DeferredRegister<Potion> POTIONS = DeferredRegister.create(ForgeRegistries.POTIONS, Habitat.MOD_ID);
    ...
    private static RegistryObject<Potion> register(String id, Supplier<StatusEffect> effect, int duration) {
        return POTIONS.register(id, () -> new Potion(new StatusEffectInstance(effect.get(), duration)));
    }

    private static RegistryObject<Potion> register(String id, Supplier<StatusEffect> effect, int duration, int amplifier) {
        return POTIONS.register(id, () -> new Potion(new StatusEffectInstance(effect.get(), duration, amplifier)));
    }
}